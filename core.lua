...
function SpecBisTooltip:GetSettingsContent(parent)
	...
	for i, invType in pairs(tab) do
		...
		if invType == "INVTYPE_FINGER" or invType == "INVTYPE_TRINKET" then
			...
			SpecBisTooltip:AppendEditbox(typ, "", function(val) end, 14, nil, true, SBTTABPC, "", text, typ)
			SpecBisTooltip:AppendEditbox(typ .. "_SOURCE", "", function(val) end, 34, nil, false, SBTTABPC, "", nil, typ)
		end
	end
end
...
local function OnTooltipSetItem(tooltip, data)
	...
	if specId then
		if icon then
			...
			if SpecBisTooltip:GetWoWBuild() == "RETAIL" then
				local foundCount = 0
				if AddToTooltipRetail(tooltip, id, specId, icon, "BISO", invType) then
					foundCount = foundCount + 1
				end

				if AddToTooltipRetail(tooltip, id, specId, icon, "BISR", invType) then
					foundCount = foundCount + 1
				end

				if AddToTooltipRetail(tooltip, id, specId, icon, "BISM", invType) then
					foundCount = foundCount + 1
				end
...
SBTSetup:SetScript(
	...
	function(self, event, ...)
		if event == "PLAYER_LOGIN" then
			...
			SpecBisTooltip:SetVersion(136031, "0.12.70")
